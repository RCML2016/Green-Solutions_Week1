export default function About() {
  return (
    <div className="bg-[#062015] text-white py-16 px-6 lg:px-14 min-h-[80vh]">
      <div className="max-w-[1000px] mx-auto">
        <div className="eyebrow">OUR VISION</div>
        <h1 className="font-display text-4xl md:text-6xl mt-4 leading-[1.05]">
          A smarter energy future <br /> starts with better <span className="text-[#22d17a]">intelligence.</span>
        </h1>
        <p className="text-white/70 max-w-2xl mt-6 text-[15px] leading-relaxed">
        ...
        <div className="grid md:grid-cols-3 gap-6 mt-14">
          {[
            ...
            <div key={c.k} className="gs-card-dark p-8">
              <div className="font-display text-3xl text-[#22d17a]">{c.k}</div>
              <p className="text-white/70 text-sm mt-4">{c.v}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-20 grid md:grid-cols-4 gap-6">
          {[
            ...
            <div key={v.n} className="border-t border-[#22d17a]/25 pt-6">
              <div className="font-mono text-[10px] text-[#6dfcb2]">{v.n}</div>
              <div className="font-display text-lg mt-3">{v.t}</div>
              <p className="text-white/60 text-sm mt-2">{v.d}</p>
            </div>
          ))}
        </div>
      </div>
