...
import { Building2, Package, Upload, History, Plus, Pencil, ArchiveRestore, Trash2, Loader2 } from "lucide-react";
...
export default function FleetAdmin() {
  ...
  return <div className="px-6 lg:px-14 py-10 max-w-full" data-testid="fleet-admin-page">
    ...
    {!loading && tab === "sites" && <Table headers={["SITE", "TYPE", "STATE", "CAPACITY", "STATUS", "ACTIONS"]}>{sites.map(s => <tr key={s.site_id} className="border-b border-[color:var(--line-2)]"><Cell mono>{s.site_id}<div className="text-[color:var(--ink)]">{s.site_name}</div></Cell><Cell>{s.site_type}</Cell><Cell>{s.state}</Cell><Cell>{Number(s.site_capacity_kW || 0).toLocaleString()} kW</Cell><Cell>{s.lifecycle_status || "active"}</Cell><Actions onEdit={() => openEdit("site", s)} onLife={() => lifecycle("site", s)} retired={s.lifecycle_status === "retired"} isAdmin={isAdmin}/></tr>)}</Table>}
    {!loading && tab === "assets" && <Table headers={["ASSET", "TYPE", "SITE", "MAKE / MODEL", "STATUS", "ACTIONS"]}>{assets.map(a => <tr key={a.asset_id} className="border-b border-[color:var(--line-2)]"><Cell mono>{a.asset_id}</Cell><Cell>{a.asset_type}</Cell><Cell>{a.site_id}<div>{a.site_name}</div></Cell><Cell>{a.make} {a.model}</Cell><Cell>{a.lifecycle_status || "active"}</Cell><Actions onEdit={() => openEdit("asset", a)} onLife={() => lifecycle("asset", a)} retired={a.lifecycle_status === "retired"} isAdmin={isAdmin}/></tr>)}</Table>}
...
function Actions({ onEdit, onLife, retired, isAdmin }) { return <td className="p-2"><div className="flex gap-2"><button title="Edit" onClick={onEdit}><Pencil size={15}/></button>{isAdmin && <button title={retired ? "Restore" : "Retire"} onClick={onLife}>{retired ? <ArchiveRestore size={15}/> : <Trash2 size={15}/>}</button>}</div></td>; }
