...
const SITE_EMPTY = { site_id: "", site_name: "", site_type: "Utility-Scale Solar", site_capacity_kW: "", state: "", address: "", city: "", zip_code: "", latitude: "", longitude: "", cod_date: "", owner_client: "" };
const ASSET_EMPTY = { asset_id: "", site_id: "", asset_type: "Inverter", make: "", model: "", serial_number: "", nameplate_kW: "", install_date: "", status: "Active", zip_code: "", latitude: "", longitude: "" };
...
export default function FleetAdmin() {
  ...
  const openEdit = (kind, row) => { setEditor({ kind, create: false }); setForm({ ...row, version: row.version || 1 }); };
  const clean = (kind) => {
    const out = { ...form };
  ...
  return <div className="px-6 lg:px-14 py-10 max-w-full" data-testid="fleet-admin-page">
    ...
    {!loading && tab === "sites" && <Table headers={["SITE", "TYPE", "LOCATION", "CAPACITY", "STATUS", "ACTIONS"]}>{sites.map(s => <tr key={s.site_id} className="border-b border-[color:var(--line-2)]"><Cell mono>{s.site_id}<div className="text-[color:var(--ink)]">{s.site_name}</div></Cell><Cell>{s.site_type}</Cell><Cell>{s.state}{(s.city || s.zip_code) && <div className="text-[10px] text-[color:var(--ink-3)]">{s.city}{s.city && s.zip_code ? ", " : ""}{s.zip_code}</div>}</Cell><Cell>{Number(s.site_capacity_kW || 0).toLocaleString()} kW</Cell><Cell>{s.lifecycle_status || "active"}</Cell><Actions onEdit={() => openEdit("site", s)} onLife={() => lifecycle("site", s)} onFeatured={() => toggleFeatured("site", s)} featured={s.featured_for_demo} retired={s.lifecycle_status === "retired"} isAdmin={isAdmin}/></tr>)}</Table>}
    ...
    {editor && <Modal title={`${editor.create ? "New" : "Edit"} ${editor.kind}`} onClose={() => setEditor(null)}><form onSubmit={save}><div className="grid md:grid-cols-2 gap-4">{editor.kind === "site" ? <><Field label="Site ID" name="site_id" value={form.site_id} onChange={change} disabled={!editor.create}/><Field label="Name" name="site_name" value={form.site_name} onChange={change}/><Field label="Category" name="site_type" value={form.site_type} onChange={change}/><Field label="Capacity (kW)" name="site_capacity_kW" type="number" value={form.site_capacity_kW} onChange={change}/><Field label="State" name="state" value={form.state} onChange={change}/><Field label="Address" name="address" value={form.address} onChange={change}/><Field label="City" name="city" value={form.city} onChange={change}/><Field label="ZIP code" name="zip_code" value={form.zip_code} onChange={change}/><Field label="COD date" name="cod_date" type="date" value={form.cod_date} onChange={change}/><Field label="Latitude" name="latitude" type="number" value={form.latitude} onChange={change}/><Field label="Longitude" name="longitude" type="number" value={form.longitude} onChange={change}/><Field label="Owner / client" name="owner_client" value={form.owner_client} onChange={change}/></> : <><Field label="Asset ID" name="asset_id" value={form.asset_id} onChange={change} disabled={!editor.create}/><label className="text-xs">Site<select className="gs-input mt-1 w-full" name="site_id" value={form.site_id} onChange={change}>{sites.map(s=><option key={s.site_id} value={s.site_id}>{s.site_id} · {s.site_name}</option>)}</select></label><Field label="Type" name="asset_type" value={form.asset_type} onChange={change}/><Field label="Make" name="make" value={form.make} onChange={change}/><Field label="Model" name="model" value={form.model} onChange={change}/><Field label="Serial number" name="serial_number" value={form.serial_number} onChange={change}/><Field label="Nameplate (kW)" name="nameplate_kW" type="number" value={form.nameplate_kW} onChange={change}/><Field label="Install date" name="install_date" type="date" value={form.install_date} onChange={change}/><Field label="ZIP override (optional)" name="zip_code" value={form.zip_code} onChange={change}/><Field label="Latitude override" name="latitude" type="number" value={form.latitude} onChange={change}/><Field label="Longitude override" name="longitude" type="number" value={form.longitude} onChange={change}/></>}</div>{editor.kind === "asset" && <p className="text-[10px] text-[color:var(--ink-3)] mt-2">Leave location overrides blank to inherit weather from the parent site.</p>}<div className="flex justify-end gap-2 mt-6"><button type="button" className="gs-btn-secondary" onClick={() => setEditor(null)}>Cancel</button><button className="gs-btn-primary">Save</button></div></form></Modal>}
  </div>;
}
